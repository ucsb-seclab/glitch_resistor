...
Function *FunctionFetcherHelper::getReadFunction(void) {
  if (this->readFunction == nullptr) {
    ...
    FunctionType *readFunctionType = FunctionType::get(
        IntegerType::getInt32Ty(m.getContext()), parameterTypes, false);
    // get the reference to function
    Function *func = cast<Function>(
        m.getOrInsertFunction(INTEREADFUNCNAME, readFunctionType));
...
Function *FunctionFetcherHelper::getReadHelperFunction(Type *targetType) {
  if (this->typeReadHelpers.find(targetType) == this->typeReadHelpers.end()) {
    ...
    FunctionType *readFunctionType =
        FunctionType::get(targetType, parameterTypes, false);
    Function *func = nullptr;
    // is this a char type?
    if (targetType->isIntegerTy(8)) {
      func = cast<Function>(
          m.getOrInsertFunction(CHARREADHELPERFUNCNAME, readFunctionType));
    } else if (targetType->isPointerTy()) {
      // is this a pointer type?
      func = cast<Function>(
          m.getOrInsertFunction(PTRREADHELPERFUNCNAME, readFunctionType));
    } else if (targetType->isIntegerTy()) {
      // is this an integer type.
      func = cast<Function>(
          m.getOrInsertFunction(INTREADHELPERFUNCNAME, readFunctionType));
    }
...
Function *FunctionFetcherHelper::getWriteHelperFunction(Type *targetType) {
  if (this->typeWriteHelpers.find(targetType) == this->typeWriteHelpers.end()) {
    ...
    FunctionType *writeFunctionType = FunctionType::get(
        IntegerType::getVoidTy(m.getContext()), parameterTypes, false);
    Function *func = nullptr;
    // is this a char type?
    if (targetType->isIntegerTy(8)) {
      func = cast<Function>(
          m.getOrInsertFunction(CHARWRITEHELPERFUNCNAME, writeFunctionType));
    } else if (targetType->isPointerTy()) {
      // is this a pointer type?
      func = cast<Function>(
          m.getOrInsertFunction(PTRWRITEHELPERFUNCNAME, writeFunctionType));
    } else if (targetType->isIntegerTy()) {
      // is this an integer type.
      func = cast<Function>(
          m.getOrInsertFunction(INTWRITEHELPERFUNCNAME, writeFunctionType));
    }
    assert(func != nullptr && "Invalid helper function type requested.");
...
Function *FunctionFetcherHelper::getWriteFunction(void) {
  if (this->writeFunction == nullptr) {
    ...
    // get the reference to function
    Function *func = cast<Function>(
        m.getOrInsertFunction(INTEWRITEFUNCNAME, readFunctionType));
...
bool FunctionFetcherHelper::isFunctionSafeToModify(const Function *currF) {
  ...
  std::set<std::string> instrumenterFunctions = {
      INTEREADFUNCNAME, INTREADHELPERFUNCNAME, CHARREADHELPERFUNCNAME,
      PTRREADHELPERFUNCNAME, INTEWRITEFUNCNAME};
  if (!currF->isDeclaration()) {
    std::string funcName;
    if (currF->hasName()) {
      funcName = currF->getName();
      return instrumenterFunctions.find(funcName) ==
             instrumenterFunctions.end();
    }
    return true;
  }
  return false;
