...
bool IntegrityCodeInserter::protectData(Value *src, Value *srcInt) {
  ...
  llvm::errs()
      << "[-] Unable to know replacement strategry for the instructions:"
      << *src << " and " << *srcInt << "\n";
...
bool IntegrityCodeInserter::canReplicateUsingReplacement(Value *currValue) {
  return dyn_cast<GetElementPtrInst>(currValue) ||
         dyn_cast<CastInst>(currValue) || dyn_cast<PHINode>(currValue);
}

bool IntegrityCodeInserter::replicateAndIntegrityProtect(Value *srcInstr,
                                                         Value *srcIntInstr) {
  ...
  for (auto U : srcInstr->users()) {
    ...
    if (canReplicateUsingReplacement(U)) {
      // next set up the insertion point to be right after the original
      // instruction.
      ...
      std::string newValueName =
          targetInstr->getValueName()->first().str() + "_gpint";
    ...
    if (auto srcGEP = dyn_cast<GEPOperator>(U)) {
      ...
      assert(
          pointerOpFound &&
          "Expected a GEP instruction to find a pointer but did not find one.");
      ...
      Value *newGEPInstr =
          builder.CreateGEP(srcIntInstr, arrIdxVals, newValueName);
    ...
    if (auto srcLoad = dyn_cast<LoadInst>(U)) {
      ...
      Function *targetFunc =
          this->fetchHelper.getReadHelperFunction(srcLoad->getType());
      ...
      if (srcLoad->getType()->isPointerTy()) {
        newVal = builder.CreatePointerCast(newVal, srcLoad->getType(),
                                           "gpConToOrPtr");
      }
      ...
      // NO need to propagate this information, because we replaced all the
      // instructions that use the load with the result of the integrity load
      // call.
      continue;
    }
    ...
    if (auto srcStore = dyn_cast<StoreInst>(U)) {
      ...
      Function *targetWriteFunction =
          this->fetchHelper.getWriteHelperFunction(toStoreValue->getType());
    ...
    if (BitCastOperator *BCO = dyn_cast<BitCastOperator>(U)) {
      ...
      Value *integrityCastInstr =
          builder.CreateBitCast(srcIntInstr, BCO->getDestTy(), "gpintCast");
...
Value *IntegrityCodeInserter::createNewLocalVar(Function *srcFunction,
                                                Type *varType) {
  IRBuilder<> builder(&(*(srcFunction->getEntryBlock().getFirstInsertionPt())));
  AllocaInst *newAlloca =
      builder.CreateAlloca(varType, nullptr, "intProtectTmp");
  return newAlloca;
}

bool IntegrityCodeInserter::replaceUsesOfInstruction(Instruction *currInstr,
                                                     Value *newVal) {
...
bool IntegrityCodeInserter::protectCallInstr(Value *srcInstr,
                                             Value *srcIntInstr, CallInst *CI) {
  ...
  if (CI != nullptr) {
    ...
    if (calledFunc != nullptr && calledFunc->hasName() &&
      ...
      if (funcName.find("scanf") != std::string::npos) {
        assert(
            srcInstr->getType()->isPointerTy() &&
            "We are passing non-pointer argument to scanf. This is strange.");
        ...
        Value *tmpVariable = createNewLocalVar(
            calleeFunction, srcIntType->getPointerElementType());
        ...
        DataLayout DL(&m);
        unsigned typeSize =
            DL.getTypeStoreSize(srcIntType->getPointerElementType());
        ...
        Value *sizeToWrite =
            ConstantInt::get(IntegerType::getInt32Ty(m.getContext()), typeSize);
        Function *targetIntWriteFunction = fetchHelper.getWriteFunction();
        Value *arguments[] = {srcVoidPtr, srcIntVoidPtr, tmpVoidPtr,
                              sizeToWrite};
        // insert the call.
        ...
        if (CI->getArgOperand(0)->stripPointerCasts() ==
            srcInstr->stripPointerCasts()) {
          // we are storing into the variable.
          Value *srcVoidPtr = insertVoidPtrCast(srcInstr, builder);
          Value *srcIntVoidPtr = insertVoidPtrCast(srcIntInstr, builder);
          Value *tmpVoidPtr = insertVoidPtrCast(CI->getArgOperand(1), builder);
          Function *targetIntegrityWriteFunction =
              fetchHelper.getWriteFunction();
          // convert the size argument into correct type
          Value *memcpySizeArg = builder.CreateIntCast(
              CI->getArgOperand(2), IntegerType::getInt32Ty(m.getContext()),
              false, "gpIntCast");
          Value *arguments[] = {srcVoidPtr, srcIntVoidPtr, tmpVoidPtr,
                                memcpySizeArg};
          // insert the call.
          ...
          isHandled = true;
        } else if (CI->getArgOperand(1)->stripPointerCasts() ==
                   srcInstr->stripPointerCasts()) {
...
Value *IntegrityCodeInserter::insertVoidPtrCast(Value *srcPtr,
                                                IRBuilder<> &builder) {
  return builder.CreatePointerCast(srcPtr, Type::getInt8PtrTy(m.getContext()),
                                   "gpProtectCast");
}
